...
Throughout this thesis several scales are used to evaluate and estimate hydrophobic values of peptides. All the scales aim for quantifying the hydrophobic values of each residue. There are several key differences in their methodology, assumptions, and aims.Crucially this results in slightly different scores for some residues. Because of this, it's preferable, and typical amongst the literature, to use several scales to verify the patterns observable in one scale.

\subsubsection{Kyte \& Doolittle Hydropathy Scale}
A consensus scale based on the water-vapor transfer free energies and the interior-exterior distribution of amino acid

\subsubsection{Hessa's Biological Hydrophobicity Scale}
sed on an experimental method where the recognition of artificial helices by the Sec61 translocon was measured. These measurements were then used to calculate a “biological hydrophobicity scale.” Here, we use the non- position dependent version of the scale,2 while in the original development of SCAMPI10 the position specific ver- sion

\subsubsection{White and Wimley Octanol \--- Interface Scale}
This scale is based on the partitioning of host-guest pentapeptides (acetyl-WL-X-LL-OH) and another set of peptides (AcWLm) between octanol and water.
Calculated

\subsubsection{The Eisenberg Consensus Scale}
This is a consensus scale based on the earlier scales from Tanford,19 Wolfenden,20 Chothia,21 Janin,17 and von Heijne.22 The scales were normalized using serine as the mean

\subsection{The TRANSMEM Annotation in Uniprot}

\subsubsection{Alternative scales}
